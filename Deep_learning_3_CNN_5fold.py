...
using UNet, DeepLabV3+, and PSPNet from segmentation_models_pytorch.
...
# Enable cuDNN benchmark mode for improved GPU performance.
torch.backends.cudnn.benchmark = True

# ------------------------------
# Device Setup: Force GPU usage if available
# ------------------------------
...
# ------------------------------
# Helper: Plot Confusion Matrix in %
# ------------------------------
...
# ------------------------------
# Helper: Plot Predictions and Loss Curves
# ------------------------------
...
# ------------------------------
# 5) Main Script (5-Fold CV) with Logging, Confusion Matrix, and Plotting
# ------------------------------
def main():
    # Set plot_only to True to generate plots without training.
    plot_only = False  # Change to True to only generate plots com saved checkpoints.
    ...
    # ------------------------------
    image_dir = r"D:/AGRICULTURA_MS/SOJA_SHP/MERGED_1/output/merged_fused_centralized_patches_raw_128"
    mask_dir = r"D:/AGRICULTURA_MS/SOJA_SHP/MERGED_1/output/annotations_patches_128"
    base_fig_dir = r"D:/AGRICULTURA_MS/SOJA_SHP/MERGED_1/output/prediction_figures"
    ...
    model_defs = {
        "UNet": lambda: Unet(encoder_name="resnet34", encoder_weights=None, in_channels=16, classes=1),
    ...
    for train_idx, val_idx in kf.split(np.arange(num_samples)):
        ...
        for model_name, model_fn in model_defs.items():
            ...
            checkpoint_path = f"{model_name}_fold{fold_num}_best_16band.h5"
            ...
            if plot_only:
                if os.path.exists(checkpoint_path):
                    ...
                    print(f"Loaded checkpoint for {model_name} on Fold {fold_num}")
                else:
                    print(f"Checkpoint {checkpoint_path} not found. Skipping {model_name} on Fold {fold_num}.")
                    continue
                ...
                log_lines.append(f"Final Validation Loss: {val_loss:.4f}")
                log_lines.append(f"Final IoU: {val_metrics['IoU']:.4f}")
                log_lines.append(f"Final Accuracy: {val_metrics['Accuracy']:.4f}")
                log_lines.append(f"Final F1: {val_metrics['F1']:.4f}")
                ...
                print(f"Best {model_name} IoU on Fold {fold_num}: {best_val_iou:.4f}")
